...
function loadData() {
    d3.csv("data/avg_rent_by_pop.csv"). then(data=>{
        ...
        d3.json("data/BURAK_cities_data_multi_year.json").then(data => {
            ...
            Promise.all([
                d3.csv("data/rent-prices.csv").then(rows => rows.map(row => mapRowBySchema(row, RENT_DATA_MAP))),
                d3.csv("data/u65incomedata.csv").then(rows => rows.filter(row => ['A', 'B', 'C', 'D', 'E'].includes(row.STATUS))
...
function changePage(page)   {
    ...
    if (page == 1)  {
        ...
        // Initialize rental map visualization (vis2)
        if (typeof initCityMap !== 'undefined') {
            initCityMap();
        }
    }   else if (page == 3)  {
...
// Note: initCityMap() is now called when user navigates to page 2 in changePage() function
